...
<%
	...
	if(wordDoc.generateDocument(syllabus)) {
		...
		// Create an object of the class that allows us to create the PDF document
		WordToPDFConverter converter = new WordToPDFConverter();
		// Convert the WORD document to PDF and verify success
		if(converter.convertWordToPDF(syllabus.getCode())) {
			System.out.println("The PDF was created successfully...");
			%>
			<h1>El archivo ha sido creado satisfactoriamente</h1>
			<a href="syllabusesDocuments/prontuario-<%=syllabus.getCode()%>.pdf" download onclick="setTimeout(function(){window.close();},5000);">
