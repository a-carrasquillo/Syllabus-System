...
<%
	...
	if((session.getAttribute("userName")==null) || (session.getAttribute("currentPage")==null)) {
		...
		String currentPage = "editSyllabus.jsp";
		...
		try {
			...
			if(appDBAuth.verifyUserPageAccess(userName, currentPage)) {
				...
				if(res.next()) {
					...
					session.setAttribute("currentPage", "editSyllabus.jsp");
					...
					<html lang="es">
					    <head>
					        ...
					        <link rel = "stylesheet" type="text/css" href="css/editSyllabus.css">
					        <!--Importing JS required-->
					        <script src="https://kit.fontawesome.com/7ea556f8eb.js" crossorigin="anonymous"></script>
					        <!--Website title-->
					        <title>Edici&oacute;n de Prontuarios</title>
			        ...
			        // Define the object that will hold the syllabus information
			        SyllabusInfo syllabus = null;
			        // Define a variable to hold the course code
			        String courseCode = "";
			       
			        // Check for error and load the information
			        if(previousPage.equals("courseSyllabusPreview.jsp")) {
						...
						// Search the syllabus information
						syllabus = appDBMnger.searchSyllabusInfoEdition(courseCode);
						// Define a session variable to hold the syllabus information
						session.setAttribute("syllabus", syllabus);
			        } else if((session.getAttribute("errorSyllabusEdition")!=null && (previousPage.equals("help.jsp") || previousPage.equals("processSyllabusEdition.jsp"))) || previousPage.equals("help.jsp")) {
			        	// Load information from session variables
			        	syllabus = (SyllabusInfo) session.getAttribute("syllabus");
			        }
					%>
			        <div class = "cuadro_centro">
			            <form action="processSyllabusEdition.jsp" method="post">               
			                <!--Section of course code-->
			                <div id = "course_code">
			                    <span id= course_code_tag>C&oacute;digo del Curso:</span>
			                    <input type="text" name="course_code" id="course_code_info" value="<%=syllabus.getCode()%>" readonly>
			                </div>
			                
			                <!--Section of course title-->
			                <div id="course_title">
			                    <span id = "title_tag">T&iacute;tulo del curso:</span>
			                    <input type="text" id="course_title_info" name="course_title" value="<%=syllabus.getTitle()%>" readonly>
			                </div>
			                
			                <!--Section of course type-->
			                <div id="course_type">
			                    <span id = "course_type_tag">Tipo de Curso:</span>
			                    <input type="text" id="course_type_info" name="course_type" value="<%=syllabus.getType()%>" readonly>
			                </div>
			                
			                <!--Section of modality-->
			                <div id="modality">
			                    <span id = "modality_tag">Modalidad:</span>
			                    <input type="text" id="modality_info" name="modality" value="<%=syllabus.getModality()%>" readonly>
			                </div>
			                
			                <!--Section of course level-->
			                <div id="level">
			                    <span id = "level_tag">Nivel:</span>
			                    <input type="text" id="nivel" name="nivel" value="<%=syllabus.getLevel().substring(0, 1).toUpperCase() + syllabus.getLevel().substring(1)%>" readonly>
			                </div>
			                ...
			                if(!syllabus.getPrerequisites().isEmpty()) {
			                    ...
			                    for(int i=0; i<syllabus.getPrerequisites().size(); i++) {
			                    	// Verify if the prerequisite is not the last one
			                ...
			                if(!syllabus.getCorequisites().isEmpty()) {
			                    ...
			                    for(int i=0; i<syllabus.getCorequisites().size(); i++) {
			                    	// Verify if the co-requisite is not the last one
			                ...
			                <div id="duration">
			                    ...
			                    <input type="text" id="duration_info" name="duration" placeholder="Introduzca la duraci&oacute;n del curso" value="<%=syllabus.getDuration()%>" required>
			                </div>
			                
			                <!--Section of credits-->
			                <div id="credits">
			                    <span id = "credits_tag">Cr&eacute;ditos:</span>
			                    <input type="text" id="credits_info" name="credits" placeholder="Introduzca la cantidad de cr&eacute;ditos del curso" value="<%=syllabus.getCredits()%>" required>
			                </div>
			                ...
			                <div id="objectives">
			                    ...
			                    <fieldset id="agencias">
			                        ...
			                        <input type="checkbox" name="caep" id="caep" value="caep" onclick="addCaep()" checked/> CAEP
			                    </fieldset>
			                    ...
			                    <%
			                    // Variable to hold all the CAEP standards that apply to an 
			                    // specific objective
								String caepStds = "";
								// Determine the number of alignments that the first objective has
					            int alignmentSize = syllabus.getObjectives().get(0).getObjectiveAlignment().size();
					            // Iterate over the alignments of the first objective
			                    ...
			                    <div id="add_caep">
			                    	<div class="caep_stds">
					                    ...
					                    <div class="botones_objetivos">                                
											<button class="add" name="caep_stds_add_1" type="button" onclick="addCaepStdToInputField(this.name)">A&ntilde;adir</button><br>
											<button class="remove" name="caep_stds_remove_1" type="button" onclick="removeCaepStdFromInputField(this.name)">Remover</button>
										</div>
										<select name="all_caep_stds_1" class="all_caep_stds">
											<option value="">Seleccione un est&aacute;ndar</option>
											<option value="1.1" title="Candidates demonstrate an understanding of the 10 InTASC standards at the appropriate progression level(s) in the following categories: the learner and learning; content; instructional practice; and professional responsibility.">1.1</option>
											<option value="1.2" title="Providers ensure that candidates use research and evidence to develop an understanding of the teaching profession and use both to measure their P-12 students’ progress and their own professional practice.">1.2</option>
											<option value="1.3" title="Providers ensure that candidates apply content and pedagogical knowledge as reflected in outcome assessments in response to standards of Specialized Professional Associations (SPA), the National Board for Professional Teaching Standards (NBPTS), states, or other accrediting bodies (e.g., National Association of Schools of Music – NASM).">1.3</option>
											<option value="1.4" title="Providers ensure that candidates demonstrate skills and commitment that afford all P-12 students access to rigorous college- and career-ready standards (e.g., Next Generation Science Standards, National Career Readiness Certificate, Common Core State Standards).">1.4</option>
											<option value="1.5" title="Providers ensure that candidates model and apply technology standards as they design, implement and assess learning experiences to engage students and improve learning; and enrich professional practice.">1.5</option>
											<option value="2.1" title="Partners co-construct mutually beneficial P-12 school and community arrangements, including technology-based collaborations, for clinical preparation and share responsibility for continuous improvement of candidate preparation. Partnerships for clinical preparation can follow a range of forms, participants, and functions. They establish mutually agreeable expectations for candidate entry, preparation, and exit; ensure that theory and practice are linked; maintain coherence across clinical and academic components of preparation; and share accountability for candidate outcomes.">2.1</option>
											<option value="2.2" title="Partners co-select, prepare, evaluate, support, and retain high-quality clinical educators, both provider- and school-based, who demonstrate a positive impact on candidates’ development and P-12 student learning and development. In collaboration with their partners, providers use multiple indicators and appropriate technology-based applications to establish, maintain, and refine criteria for selection, professional development, performance evaluation, continuous improvement, and retention of clinical educators in all clinical placement settings.">2.2</option>
											<option value="2.3" title="The provider works with partners to design clinical experiences of sufficient depth, breadth, diversity, coherence, and duration to ensure that candidates demonstrate their developing effectiveness and positive impact on all students’ learning and development. Clinical experiences, including technology-enhanced learning opportunities, are structured to have multiple performance-based assessments at key points within the program to demonstrate candidates’ development of the knowledge, skills, and professional dispositions, as delineated in Standard 1, that are associated with a positive impact on the learning and development of all P-12 students.">2.3</option> 
											<option value="3.1" title="The provider presents plans and goals to recruit and support completion of high-quality candidates from a broad range of backgrounds and diverse populations to accomplish their mission. The admitted pool of candidates reflects the diversity of America’s P-12 students. The provider demonstrates efforts to know and address community, state, national, regional, or local needs for hard-to-staff schools and shortage fields, currently, STEM, English-language learning, and students with disabilities.">3.1</option> 
											<option value="3.2" title="The provider meets CAEP minimum criteria or the state’s minimum criteria for academic achievement, whichever are higher, and gathers disaggregated data on the enrolled candidates whose preparation begins during an academic year. The CAEP minimum criteria are a grade point average of 3.0 and a group average performance on nationally normed assessments or substantially equivalent statenormed assessments of mathematical, reading and writing achievement in the top 50 percent of those assessed...">3.2</option> 
											<option value="3.3" title="Educator preparation providers establish and monitor attributes and dispositions beyond academic ability that candidates must demonstrate at admissions and during the program. The provider selects criteria, describes the measures used and evidence of the reliability and validity of those measures, and reports data that show how the academic and non-academic factors predict candidate performance in the program and effective teaching.">3.3</option> 
											<option value="3.4" title="The provider creates criteria for program progression and monitors candidates’ advancement from admissions through completion. All candidates demonstrate the ability to teach to college- and career-ready standards. Providers present multiple forms of evidence to indicate candidates’ developing content knowledge, pedagogical content knowledge, pedagogical skills, and the integration of technology in all of these domains.">3.4</option> 
											<option value="3.5" title="Before the provider recommends any completing candidate for licensure or certification, it documents that the candidate has reached a high standard for content knowledge in the fields where certification is sought and can teach effectively with positive impacts on P-12 student learning and development.">3.5</option> 
											<option value="3.6" title="Before the provider recommends any completing candidate for licensure or certification, it documents that the candidate understands the expectations of the profession, including codes of ethics, professional standards of practice, and relevant laws and policies. CAEP monitors the development of measures that assess candidates’ success and revises standards in light of new results.">3.6</option> 
											<option value="4.1" title="The provider documents, using multiple measures that program completers contribute to an expected level of student-learning growth. Multiple measures shall include all available growth measures (including value-added measures, student-growth percentiles, and student learning and development objectives) required by the state for its teachers and available to educator preparation providers, other state-supported P-12 impact measures, and any other measures employed by the provider.">4.1</option> 
											<option value="4.2" title="The provider demonstrates, through structured validated observation instruments and/or student surveys, that completers effectively apply the professional knowledge, skills, and dispositions that the preparation experiences were designed to achieve.">4.2</option> 
											<option value="4.3" title="The provider demonstrates, using measures that result in valid and reliable data and including employment milestones such as promotion and retention, that employers are satisfied with the completers’ preparation for their assigned responsibilities in working with P-12 students.">4.3</option> 
											<option value="4.4" title="The provider demonstrates, using measures that result in valid and reliable data, that program completers perceive their preparation as relevant to the responsibilities they confront on the job, and that the preparation was effective.">4.4</option> 
											<option value="5.1" title="The provider’s quality assurance system is comprised of multiple measures that can monitor candidate progress, completer achievements, and provider operational effectiveness. Evidence demonstrates that the provider satisfies all CAEP standards.">5.1</option> 
											<option value="5.2" title="The provider’s quality assurance system relies on relevant, verifiable, representative, cumulative and actionable measures, and produces empirical evidence that interpretations of data are valid and consistent.">5.2</option> 
											<option value="5.3" title="The provider regularly and systematically assesses performance against its goals and relevant standards, tracks results over time, tests innovations and the effects of selection criteria on subsequent progress and completion, and uses results to improve program elements and processes.">5.3</option> 
											<option value="5.4" title="Measures of completer impact, including available outcome data on P-12 student growth, are summarized, externally benchmarked, analyzed, shared widely, and acted upon in decision-making related to programs, resource allocation, and future direction.">5.4</option> 
											<option value="5.5" title="The provider assures that appropriate stakeholders, including alumni, employers, practitioners, school and community partners, and others defined by the provider, are involved in program evaluation, improvement, and identification of models of excellence. (see caepnet.org)">5.5</option> 
										</select>
					                </div>	
			                    </div>
			                        ...
			                        for(int i=1; i<syllabus.getObjectives().size(); i++) {
			                        	...
			                        	<div>
			                        		<div class="objective">
							                	...
							                	<span><textarea class="objective_info" name="objective_<%=i+1%>" placeholder="Introduzca la descripci&oacute;n del objetivo <%=i+1%>" rows="3"><%=syllabus.getObjectives().get(i).getDescription()%></textarea></span>
							                </div>
						                	<%
						                	// Reset the CAEP standards variable
						                	caepStds = "";
						                	// Determine the number of alignments that the objective 'i' has
								            ...
								            for(int j=0; j<alignmentSize; j++) {
								            	/* If more agencies are added, we need to verify the agency id, to know 
								                to which agency the objectives belongs to. For now, this is not needed */
							                ...
							                <div class="caep_stds">
							                	...
							                	<div class="botones_objetivos">
													<button class="add" name="caep_stds_add_<%=i+1%>" type="button" onclick="addCaepStdToInputField(this.name)">A&ntilde;adir</button><br>
													<button class="remove" name="caep_stds_remove_<%=i+1%>" type="button" onclick="removeCaepStdFromInputField(this.name)">Remover</button>
												</div>
												<select name="all_caep_stds_<%=i+1%>" class="all_caep_stds">
													<option value="">Seleccione un est&aacute;ndar</option>
													<option value="1.1" title="Candidates demonstrate an understanding of the 10 InTASC standards at the appropriate progression level(s) in the following categories: the learner and learning; content; instructional practice; and professional responsibility.">1.1</option>
													<option value="1.2" title="Providers ensure that candidates use research and evidence to develop an understanding of the teaching profession and use both to measure their P-12 students’ progress and their own professional practice.">1.2</option>
													<option value="1.3" title="Providers ensure that candidates apply content and pedagogical knowledge as reflected in outcome assessments in response to standards of Specialized Professional Associations (SPA), the National Board for Professional Teaching Standards (NBPTS), states, or other accrediting bodies (e.g., National Association of Schools of Music – NASM).">1.3</option>
			                    ...
			                    <div id="new_thematic_content">
			                        ...
			                        for(int i=1; i<syllabus.getThematicContent().size(); i++) {
					                    ...
					                    <div class="thematic_content">
						                    <span><%=i+1%>. </span><input type="text" class="thematic_info" name="thematic_info_<%=i+1%>" placeholder="Introduzca el contenido tem&aacute;tico" required value="<%=syllabus.getThematicContent().get(i)%>">
						                </div>
					                    <%
			                        }
			                        %>
			                ...
			                <div>
			                    <div id="teaching_strat_1">
			                        ...
			                        <input type="text" name="teaching_strat_1" class="teaching_strat_info" placeholder="Seleccione o introduzca una estrategia de ense&ntilde;anza" required list="teaching_strats" value="<%=syllabus.getTeachingStrategies().get(0)%>"/>
			                        <datalist id="teaching_strats">
					                    ...
					                    // Iterate over the result and extract the teaching strategies types
					                    while(res.next()) {
			                    ...
			                    <div id="new_teaching_strat">
			                        <%
			                        // Iterate over the teaching strategies that the syllabus already has
			                ...
			                <div>
			                    <div id="assessment_strat_1">
			                        ...
			                        <input type="text" name="assessment_strat_1" class="assessment_strat_info" placeholder="Seleccione o introduzca una estrategia de assessment" required list="assessment_strats" value="<%=syllabus.getAssessmentStrategies().get(0)%>"/> 
			                        <datalist id="assessment_strats">
			                        	<%
			                        	// Search all types of assessment strategies
			                        	// currently store in the DB
					                    res = appDBMnger.listAllTypesOfAssessment();
					                    // Iterate over the result of assessment strategies types
					                    while(res.next()) {
						                    %><option><%=res.getString(2)%></option><%
						                }
			                        	%>
			                    ...
			                    <div id="new_assessment_strat">
			                        <%
			                        // Iterate over the assessment strategies that the syllabus currently has
			                ...
			                <div>
			                    ...
			                    <div id="new_textbook">
			                        <%
			                        // Iterate over the second textbook forward
			                ...
			                <div>
			                    ...
			                    <div id="new_bibliography">
			                        <%
			                        // Iterate over the second bibliography forward
			                        for(int i=1; i<syllabus.getBibliography().size(); i++) {
					                    %>
					                    <div class="bibliography_content">
						                    <span><%=i+1%>. </span><input type="text" class="bibliography_info" name="bibliography_info_<%=i+1%>" placeholder="Introduzca la bibliograf&iacute;a" required value="<%=syllabus.getBibliography().get(i)%>">
						                </div>
					                    <%
					                }
			                        %>
			                ...
			                <div>
			                    ...
			                    <div id="new_online_res">
			                        <%
			                        // Iterate over the second online resource forward
			                ...
			                <div id="reglas"><div>Reglas que aplican:</div>
			                    <table class="regla">
							        ...
							        // Iterate over the rules
					               	while(res.next()) {
					               		// Verify if the rule is in the array list to add
					                	...
					                	// Gives the rules a structure of two columns
					                	if(counter%2 == 0) {
					                		%>
					                		<tr>
			                ...
			                <!--End Buttons-->
			                <div id="buttons">
			                    <button id ="cancel" type="button" onclick="window.location.href='courseSyllabusPreview.jsp';">Cancelar</button>
			        ...
			        <script>                                                        
			            ...
			            function addObjective() {
			                /*Here we add the variables required to verify
			                  the selection of the check boxes*/
			                var amountObjectivesLoc = document.getElementById("amountObjectives");
			                var amountObjectives = Number(amountObjectivesLoc.value);
			                amountObjectives += 1; 
			                ...
			                if (matches) { 
			                    ...
			                    if(inputField[0].value == "" && dropdown[0].value != "") {
			                        ...
			                        if(inputField[0].value.search(dropdown[0].value) == -1) {
			                            //not found  
			                            inputField[0].value = inputField[0].value.trim() + " " + dropdown[0].value;
			                        } else {
			                            //found
